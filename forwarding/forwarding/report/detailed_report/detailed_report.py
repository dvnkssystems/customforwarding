...
def execute(filters=None):
	...
	list(map(update_operations,res))
	return columns, res

def update_operations(data):
	try:
		if data['voucher_type'] and data['voucher_no']:
			data['operations']=frappe.db.get_value(data['voucher_type'],data['voucher_no'],'operations')
			if not data['project']:
				 data['project'] =data['operations']
	except:
		pass
...
def get_gl_entries(filters, accounting_dimensions):
	...
	if filters and filters.get('cost_center'):
		...
		left join 
		`tabAccount` ac
		on ac.type in ('Expense Account','Income Account')
...
def update_direct_payment_id(gl_entry):
	if gl_entry['voucher_type']=="Journal Entry":
		...
		gl_entry['voucher_no'] = frappe.db.get_value("Journal Entry",jv,"direct_payment_id")

def get_conditions(filters):
	conditions = []
	conditions.append("""account in (select name from tabAccount
		where account_type in ('Expense Account','Income Account'))""")
